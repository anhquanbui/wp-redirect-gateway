...
class WPRG_Shortcode_Inline {
    ...
    public function render_inline_button_shortcode( $atts ) {
        ...
        if ( empty( $slug ) ) return '<div class="wprg-shortcode-error">' . esc_html__( 'Error: Please enter the link slug.', 'redirect-gateway-manager' ) . '</div>';
        ...
        if ( ! isset( $shortcodes[ $sc_id ] ) ) return '<div class="wprg-shortcode-error">' . esc_html__( 'Error: Gateway has been deleted.', 'redirect-gateway-manager' ) . '</div>';
        ...
        wp_localize_script( 'wprg-gateway-js', 'wprgData', array(
            ...
            'i18n'        => array(
                ...
                'active_warning'      => __( 'You have another active link running!', 'redirect-gateway-manager' ),
                ...
                'click_to_watch'      => __( 'Click to continue', 'redirect-gateway-manager' ),
        ...
        <div id="wprg-pass-wrap-<?php echo esc_attr($slug); ?>" class="wprg-gateway-wrapper wprg-theme-inline">
            <div class="wprg-password-container">
                <h4 class="wprg-title wprg-text-danger">🔒 <?php esc_html_e( 'Password Required', 'redirect-gateway-manager' ); ?></h4>
                ...
                <form class="wprg-ajax-pass-form" data-slug="<?php echo esc_attr($slug); ?>">
                    <input type="password" class="wprg-pass-input" placeholder="<?php esc_attr_e( 'Enter password...', 'redirect-gateway-manager' ); ?>" required>
                    <button type="submit" class="wprg-pass-submit"><?php esc_html_e( 'UNLOCK BUTTON', 'redirect-gateway-manager' ); ?></button>
                </form>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div id="wprg-btn-wrap-<?php echo esc_attr($slug); ?>" class="wprg-gateway-wrapper wprg-theme-inline" data-slug="<?php echo esc_attr($slug); ?>" data-wait="<?php echo esc_attr($final_wait_time); ?>" data-ads="<?php echo intval( $link_data['ad_count'] ); ?>" data-logid="0" style="<?php echo ( ! $is_unlocked ) ? 'display:none;' : ''; ?>">
            <p class="wprg-title wprg-fw-bold"><?php esc_html_e( 'Click the button below to get the link:', 'redirect-gateway-manager' ); ?></p>
            <button class="wprg-action-btn">
                <?php esc_html_e( 'START GETTING LINK', 'redirect-gateway-manager' ); ?>
            </button>
